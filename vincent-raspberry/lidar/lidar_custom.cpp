...
u_result capture_and_display(RPlidarDriver * drv, const char* fname)
{
    ...
    if (IS_OK(ans) || ans == RESULT_OPERATION_TIMEOUT) {
        ...
        for (int pos = 0; pos < (int)count ; ++pos) {
            ...
            float endptY = (distPixel*cos(rad))+centerPtY; //chnage this
        ...
        //print data into arrow.dat
        //float angleFace = (float)((*angle_moved)*PI/180.0);
        FILE* someFile = fopen("arrow.dat", "w");
        fprintf(someFile, "%d\t%d\t\n", (int)centerPtX, (int)centerPtY);
        /*fprintf(someFile, "%f\t%f\t\n", ((-2)*cos(angleFace) + (-2)*sin(angleFace) + centerPtX),
        ((-2)*cos(angleFace) +(-2)*sin(angleFace) + centerPtY));
        fprintf(someFile, "%f\t%f\t\n", (2*cos(angleFace) + (-2)*sin(angleFace) + centerPtX),
        ((-2)*cos(angleFace) + 2 * sin(angleFace) + centerPtY));
        */
        //fprintf(someFile, "%f\t%f\t\n", 3*sin(angleFace+195*PI/180.0) + centerPtX, 3*cos(angleFace+195*PI/180.0) + centerPtY);
        //fprintf(someFile, "%f\t%f\t\n", 3*sin(angleFace+165*PI/180.0) + centerPtX, 3*cos(angleFace+165*PI/180.0) + centerPtY);
